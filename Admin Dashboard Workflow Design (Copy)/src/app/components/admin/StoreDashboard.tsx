...
import { ChevronLeft, MapPin, Cpu, CheckCircle, XCircle, AlertCircle, ChevronRight, FileText, X, Clock } from 'lucide-react';
import { PieChart, Pie, Cell } from 'recharts';
import { stores, Round, ScanLog } from '../../mockData';
...
/** Parse "09:00 AM" → total minutes since midnight */
...
function getRoundTimes(startTime: string) {
  const startMins = parseTime(startTime);
  const endMins   = startMins + 90;
  // mock "current" time: 09:37 AM = 577 min
  const nowMins   = 9 * 60 + 37;
  const remaining = Math.max(0, endMins - nowMins);
  return {
    endTime: formatTime(endMins),
    remaining: remaining > 0 ? `${remaining} min remaining` : 'Overdue',
    isOverdue: remaining === 0,
  };
}

function ScanLogModal({ round, onClose }: { round: Round; onClose: () => void }) {
  const t = useT();
  return (
          ...
          {round.scans.map((scan: ScanLog) => (
            <div key={scan.id} className={`flex items-center justify-between p-3 rounded-xl border ${
              ...
              <div className="text-right">
                <div className={`text-xs ${t.textSm}`}>{scan.time}</div>
                <div className={`text-xs font-medium capitalize ${
                  scan.status === 'verified' ? 'text-green-600' : scan.status === 'missed' ? 'text-red-600' : t.textMuted
                }`}>{scan.status}</div>
              </div>
            </div>
          ))}
...
export function StoreDashboard({ storeId, goBack, navigate }: Props) {
  ...
  const tagStatusCount = {
    ...
    pending: store.tags.filter(tag => tag.status === 'pending').length,
  ...
  return (
    <div className={`flex-1 overflow-y-auto pb-6 ${t.page}`}>
      ...
      <div className="px-4 pt-4 space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className={t.textXs}>Green – On Track</span>
                <span className="font-medium text-green-600">≥85%</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className={t.textXs}>Amber – At Risk</span>
                <span className="font-medium text-amber-600">70–84%</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className={t.textXs}>Red – Critical</span>
        ...
        {/* Rounds — with end time + remaining */}
        <div className={`rounded-2xl border shadow-sm overflow-hidden ${t.card}`}>
          <div className={`px-4 py-3 border-b ${t.border}`}>
            <h3 className={`font-semibold text-sm ${t.text}`}>Round Details</h3>
            <p className={`text-xs ${t.textXs}`}>Tap a round to view scrollable scan log</p>
          </div>
          {store.rounds.map((round, i) => {
            const { endTime, remaining, isOverdue } = getRoundTimes(round.time);
            return (
              <button key={round.id} onClick={() => setActiveRound(round)}
                className={`w-full px-4 py-4 flex items-center justify-between ${t.hoverRow} text-left transition-colors ${i > 0 ? `border-t ${t.borderGray}` : ''}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold text-white ${
                    round.compliance >= 85 ? 'bg-green-500' : round.compliance >= 70 ? 'bg-amber-500' : 'bg-red-500'
                  }`}>
                    {round.compliance}%
                  </div>
                  <div>
                    ...
                    <div className={`flex items-center gap-1 text-xs mt-0.5 ${t.textMuted}`}>
                      ...
                      {round.time} → {endTime}
                      <span className={`ml-1 ${isOverdue ? 'text-red-500 font-medium' : 'text-green-600'}`}>
                        ({remaining})
                      </span>
                    </div>
